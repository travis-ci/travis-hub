    ...
    include do
      ...
      protected
        ...
        def decode(payload)
          ...
          debug "[#{Thread.current.object_id}] [decode error] payload could not be decoded with engine #{MultiJson.engine.to_s} : #{(payload.size > 160 ? "#{payload[0..160]} ..." : payload)}"
          debug "[#{Thread.current.object_id}] [decode error] #{e.inspect}"
          nil
        end
