describe Travis::Addons::Serializer::Pusher::Job do
  ...
  let(:job)    { FactoryBot.create(:job, repository: repo, build: build, commit: commit, stage: stage) }
  let(:commit) { FactoryBot.create(:commit) }
  let(:build)  { FactoryBot.create(:build) }
  let(:stage)  { FactoryBot.create(:stage, build: build, number: 1, name: 'test', state: :created) }
