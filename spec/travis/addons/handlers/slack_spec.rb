describe Travis::Addons::Handlers::Slack do
  ...
  let(:build)   { FactoryBot.create(:build, state: :passed, config: { notifications: { slack: config } }) }
  let(:config)  { 'room' }
  ...
  describe 'multiple configs' do
    ...
    let(:jobs)    { Sidekiq::Queues.jobs_by_queue['slack'] }
  ...
  describe 'given a plain string' do
    let(:params) { JSON.parse(Sidekiq::Queues.jobs_by_queue['slack'][0]['args'].last) }
    let(:config) { 'room' }
