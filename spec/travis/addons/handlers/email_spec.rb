describe Travis::Addons::Handlers::Email do
  ...
  let(:build)   { FactoryBot.create(:build, repository: repo, commit:, config: { notifications: { email: config } }) }
  ...
  describe 'handle?' do
    context 'when auto-canceling build' do
      ...
      let(:handler) { described_class::Notifier.new('build:canceled', id: build.id, config:) }
